...
[`docs/ACCOUNTS.md`](Token-Footprint/docs/ACCOUNTS.md).
...
See [`METHODOLOGY.md`](Token-Footprint/METHODOLOGY.md) for full formulas, sources, and
...
| `ENGINEERING_PRIOR` | A Token Footprint assumption for an unmeasured model |
...
```bash
git clone https://github.com/praveenkumar27it-stack/Token-Footprint.git
cd Token-Footprint/Token-Footprint
```
...
4. Select the `Token-Footprint` folder (the one containing `manifest.json`)

No server or configuration is needed — the extension is fully local.
Refresh any ChatGPT / Claude / Gemini tab that was open before loading it.
...
cd stats-site
npm install
npm run dev -- --host localhost   # http://localhost:5173
```

The **Add to Chrome** button opens a local install page (`#/install`) with the
steps above and the exact folder path to select.
...
See [`docs/TOKEN_CUTTER.md`](Token-Footprint/docs/TOKEN_CUTTER.md) for the
full design.
...
1. Deploy the Worker in [`proxy/`](Token-Footprint/proxy/README.md) and set the Gemini key as a
...
cd stats-site
npm install
npm run dev -- --host localhost               # http://localhost:5173 (demo data)
...
the `Token-Footprint` directory containing `manifest.json`).
...
- [`docs/PRIVACY.md`](Token-Footprint/docs/PRIVACY.md) — full privacy policy.
- [`docs/TERMS.md`](Token-Footprint/docs/TERMS.md) — terms of service (first draft).
- [`docs/DATA_DELETION.md`](Token-Footprint/docs/DATA_DELETION.md) — how to delete local and account data.
- [`docs/SECURITY.md`](Token-Footprint/docs/SECURITY.md) — security model and how to report issues.
- [`docs/CHROME_STORE_LISTING.md`](Token-Footprint/docs/CHROME_STORE_LISTING.md) — store listing copy,
  permission justifications, and data-use disclosure answers.
- [`docs/ACCOUNTS.md`](Token-Footprint/docs/ACCOUNTS.md) — optional login/sync design.
- [`docs/BACKEND_DEPLOYMENT.md`](Token-Footprint/docs/BACKEND_DEPLOYMENT.md) — provision Supabase and wire keys.
- [`docs/TESTING.md`](Token-Footprint/docs/TESTING.md) — manual test steps.
...
<https://github.com/praveenkumar27it-stack/Token-Footprint>.

## License

MIT — see [`Token-Footprint/LICENSE`](Token-Footprint/LICENSE).

...
2. Enable verbose logs: open the toolbar popup → turn on **Debug logging (dev)** (or run
   ...
   in a chat tab's console and reload). Logs are prefixed `[Token Footprint]`.
...
## Branding
- The optimizer chip uses the Token Footprint palette (cream surface, earthy-green accents,

// Token Footprint Environmental Profiles (versioned)
// ---------------------------------------------------------------------------
...
//   ENGINEERING_PRIOR Token Footprint's own assumption for a current model that
...
(function (root) {
  ...
  const SHORT_PRIORS = SHORT_PRIOR_SPECS.map(([model, provider, low, high, central, basis, sourceIds]) => ({
    ...
    basis,
    notes: [
      'Token Footprint engineering prior, not a measurement: no provider publishes production telemetry for this model.',
